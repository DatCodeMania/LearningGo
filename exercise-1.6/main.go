...
var palette = []color.Color{color.RGBA{0x00, 0xFF, 0x00, 0xFF}, color.White, color.Black, color.RGBA{0x00, 0x00, 0xFF, 0xFF}}
...
func lissajous(out *os.File) {
	...
	randomColorIndex := rand.Intn(3) + 1
	...
	for i := 0; i < nframes; i++ {
		...
		for t := 0.0; t < cycles*2*math.Pi; t += res {
			...
			img.SetColorIndex(size+int(x*size+0.5), size+int(y*size+0.5), uint8(randomColorIndex))
		}
		phase += 0.1
